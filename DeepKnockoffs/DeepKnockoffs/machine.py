        ...
        if self.DELTA > 0:
            ...
            if self.use_weighting:
                ...
                if self.diff_decorr:
                    ...
                    second_order = GaussianKnockoffs(Sigma.data.cpu().numpy(), mu=np.mean(X.data.numpy(),0), method="sdp")

                    corr_XXk = norm(torch.diag(Sigma,0) - 1 + torch.from_numpy(second_order.Ds).float()).pow(2)
                else:
